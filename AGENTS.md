...
- 对外名称统一使用 **ShotShot**，产品名与技术命名空间统一使用 `shotshot`；仓库地址为 `https://github.com/rendylong/shotshot.ai`（代码内以 `REPO_URL` 常量为准）。不得新增其他品牌名、旧域名或旧仓库地址。
...
- README 保持简洁，只放项目介绍、核心功能、快速开始、BYOK 说明、重要提示与开源致谢；对外统一使用 ShotShot 品牌。
