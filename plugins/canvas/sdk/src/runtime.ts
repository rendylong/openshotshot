...
type RuntimeGlobal = { ShotshotRuntime?: PluginRuntime };

/** 取宿主注入的插件运行时(含 React、事件总线、injectCSS 等)。 */
export function getRuntime(): PluginRuntime {
    const runtime = (globalThis as unknown as RuntimeGlobal).ShotshotRuntime;
    if (!runtime) {
        throw new Error("[plugin-sdk] shotshot 运行时未就绪:请在画布宿主中加载本插件");
    }
    return runtime;
}
...
// 这样插件作者可以直接 `import { useState } from "@shotshot/plugin-sdk"`,
