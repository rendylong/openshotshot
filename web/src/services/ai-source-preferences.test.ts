...
describe("AI source preferences", () => {
    it("accepts a separate platform choice without an API key", () => {
        expect(parseAiSourcePreferences({version: 1, selections: {agent: {source: "platform", modelId: "model"}}}).selections.agent?.source).toBe("platform");
    });
    it.each([
        ...
        {version: 1, selections: {other: {source: "platform", modelId: "model"}}},
