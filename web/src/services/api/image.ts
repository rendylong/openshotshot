...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    ...
    const quality = script ? normalizeQuality(config.quality) : undefined;
    const requestSize = script ? resolveRequestSize(quality, config.size) : config.size;
    const background = script ? normalizeBackground(config.background) : undefined;
    try {
        const result = await generateResolvedMedia({
            config,
            modality: "image",
            prompt: script ? withSystemPrompt(requestConfig, prompt) : prompt,
            params: { size: requestSize, quality: script ? quality : config.quality, count: n, ...(background ? { background } : {}) },
            signal: options?.signal,
        });
        if (result.kind !== "image") throw new Error(apiText("noImageReturned"));
        return result.sources.map((dataUrl) => ({ id: nanoid(), dataUrl }));
    } catch (error) {
        throw new Error(readAxiosError(error, apiText("requestFailed")));
    }
}
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    ...
    try {
        const result = await generateResolvedMedia({
            config,
            modality: "image",
            prompt: script ? withSystemPrompt(requestConfig, requestPrompt) : requestPrompt,
            images,
            params: {
                size: requestSize,
                quality: script ? quality : config.quality,
                count: n,
                ...(background ? { background } : {}),
                ...(!script && mask ? { mask: await imageToDataUrl(mask) } : {}),
            },
            signal: options?.signal,
        });
        if (result.kind !== "image") throw new Error(apiText("noImageReturned"));
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    await assertByokGenerationAllowed("text");
    ...
    try {
        ...
        const provider = resolveProvider(modelProviderOf(config, selectedModel), requestConfig.baseUrl, requestConfig.apiFormat);
        if (provider === "minimax-cn" || provider === "minimax-global" || provider === "deepseek" || provider === "moonshot" || provider === "zhipu" || provider === "hiapi" || provider === "openrouter") {
            const catalog = resolveModelExecution(config, selectedModel)?.catalog;
            const answer = (await requestChatCompletion(requestConfig, provider, withSystemMessage(requestConfig, messages) as AiTextMessage[], onDelta, options?.signal, { supportsImageInput: catalog?.version === 1 && catalog.inputModalities?.includes("image") === true })) || apiText("noContent");
            if (answer === apiText("noContent")) onDelta(answer);
            return answer;
        }
...
export async function fetchImageModels(config: Pick<AiConfig, "baseUrl" | "apiKey" | "apiFormat"> & { provider?: ModelChannel["provider"] }) {
    if (isAutodlChannel(config)) return AUTODL_WORKFLOWS.map(({ id }) => id);
    try {
        ...
        const modelsUrl = providerModelsUrl(resolveProvider(config.provider, config.baseUrl, config.apiFormat), config.baseUrl.trim());
...
export async function fetchChannelModels(channel: ModelChannel) {
    return fetchImageModels({ baseUrl: channel.baseUrl, apiKey: channel.apiKey, apiFormat: channel.apiFormat, provider: channel.provider });
}
