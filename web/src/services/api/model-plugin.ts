import { prepareReferenceImages, hasReferenceMask } from "../reference-image-preparation";
...
export type RunRemoteSubmitPluginArgs = Pick<RunPluginArgs, "capability" | "script" | "config" | "prompt" | "images" | "params" | "signal">;
export type RunRemoteQueryPluginArgs = Pick<RunPluginArgs, "capability" | "script" | "config" | "signal"> & { taskId: string };
 ...
 */
async function runPluginWithLocals<T>(args: Pick<RunPluginArgs, "script" | "config" | "signal">, names: string[], values: unknown[], shadowed: string[] = []): Promise<T> {
    const runner = new Function(...names, ...shadowed, `"use strict"; return (async () => {\n${args.script}\n})();`) as (...fnArgs: unknown[]) => Promise<T>;
    try {
        return await runner(...values, ...shadowed.map(() => undefined));
    } catch (error) {
        ...
        if (error && typeof error === "object" && "code" in error && typeof (error as { code?: unknown }).code === "string") {
            (wrapped as Error & { code: string }).code = (error as { code: string }).code;
        }
        throw wrapped;
    }
}
...
export async function runModelPlugin<T = unknown>(args: RunPluginArgs): Promise<T> {
    ...
    return runPluginWithLocals<T>(args,
        ["prompt", "images", "messages", "params", "model", "baseUrl", "apiKey", "systemPrompt", "reasoningEffort", "http", "request", "poll", "sleep", "signal", "onDelta"],
        [args.prompt || "", images, args.messages || [], args.params || {}, config.model, baseUrl, config.apiKey, config.systemPrompt || "", config.reasoningEffort, createPluginHttp(config, { signal: args.signal }), createPluginRequest(config, { signal: args.signal }), createPoll(args.signal), (ms: number) => sleep(ms, args.signal), args.signal, args.onDelta],
        ["taskId"],
    );
}

export async function runRemoteSubmitPlugin<T = unknown>(args: RunRemoteSubmitPluginArgs): Promise<T> {
    const { config } = args;
    const baseUrl = config.baseUrl.trim().replace(/\/+$/, "");
    const images = await prepareReferenceImages(config, args.images || [], { signal: args.signal, preserveOriginal: hasReferenceMask(args.params) });
    return runPluginWithLocals<T>(args,
        ["prompt", "images", "params", "model", "baseUrl", "apiKey", "request", "http", "signal"],
        [args.prompt || "", images, args.params || {}, config.model, baseUrl, config.apiKey, createPluginRequest(config, { signal: args.signal }), createPluginHttp(config, { signal: args.signal }), args.signal],
        ["taskId", "messages", "systemPrompt", "reasoningEffort", "poll", "sleep", "onDelta"],
    );
}

export async function runRemoteQueryPlugin<T = unknown>(args: RunRemoteQueryPluginArgs): Promise<T> {
    const { config } = args;
    const baseUrl = config.baseUrl.trim().replace(/\/+$/, "");
    return runPluginWithLocals<T>(args,
        ["taskId", "model", "baseUrl", "apiKey", "http", "request", "signal"],
        [args.taskId, config.model, baseUrl, config.apiKey, createPluginHttp(config, { signal: args.signal }), createPluginRequest(config, { signal: args.signal }), args.signal],
        ["prompt", "images", "messages", "params", "systemPrompt", "reasoningEffort", "poll", "sleep", "onDelta"],
    );
}
...
export function getRemoteTaskPluginVariables(phase: "submit" | "query", capability: ModelCapability): PluginVariable[] {
    const shared = getPluginVariables().filter((variable) => !variable.capabilities || variable.capabilities.includes(capability));
    if (phase === "submit") return shared.filter((variable) => ["prompt", "images", "params", "model", "baseUrl", "apiKey", "request", "http", "signal"].includes(variable.name));
    return [
        { name: "taskId", type: "string", desc: i18n.t("modelPlugin.variables.taskId") },
        ...shared.filter((variable) => ["model", "baseUrl", "apiKey", "http", "request", "signal"].includes(variable.name)),
    ];
}
