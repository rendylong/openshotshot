...
import { isRemoteResultRetry } from "@/lib/canvas/remote-media-task-ui";
import { useEffect, useMemo, useState, type CSSProperties, type ReactNode } from "react";
import { App, Modal, Segmented, Tooltip } from "antd";
import { Download, Ellipsis, FolderPlus, Image as ImageIcon, Info, MessageSquare, Minus, Plus, RefreshCw, Settings2, Trash2, Upload } from "lucide-react";
...
export function CanvasNodeHoverToolbar({
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...(canRetry ? [{ id: "retry", title: t(isRemoteResultRetry(node, remoteTasks) ? "canvas.remoteTask.retryResultHint" : "canvas.nodeToolbar.retryTitle"), label: t(isRemoteResultRetry(node, remoteTasks) ? "canvas.remoteTask.retryResult" : "canvas.node.retry"), icon: <RefreshCw className="size-4" />, onClick: () => onRetry(node) }] : []),
        ...
        ...(isVideo ? [{ id: "uploadVideo", title: t(hasVideo ? "canvas.nodeToolbar.replaceVideo" : "canvas.nodeToolbar.uploadVideo"), label: t(hasVideo ? "canvas.nodeToolbar.replaceVideo" : "canvas.nodeToolbar.uploadVideo"), icon: <Upload className="size-4" />, onClick: () => onUpload(node) }] : []),
        ...(isAudio ? [{ id: "uploadAudio", title: t(hasAudio ? "canvas.nodeToolbar.replaceAudio" : "canvas.nodeToolbar.uploadAudio"), label: t(hasAudio ? "canvas.nodeToolbar.replaceAudio" : "canvas.nodeToolbar.uploadAudio"), icon: <Upload className="size-4" />, onClick: () => onUpload(node) }] : []),
    ...
    return (
        <>
            <div
                className="absolute z-[70] flex h-12 -translate-x-1/2 -translate-y-full items-center overflow-visible rounded-2xl border backdrop-blur-[8px] text-sm"
                style={{ left, top, background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.toolbar.item, boxShadow: "var(--elevation-menu)" }}
                    ...
                    {view === "info" ? (
                        <div className="thin-scrollbar h-full space-y-3 overflow-auto pr-1">
                            ...
                            {node.metadata?.errorDetails ? (
                                <div className="rounded-lg border p-3 text-danger" style={{ borderColor: theme.node.stroke }}>
                                    {node.metadata.errorDetails}
                                </div>
                            ) : null}
                        </div>
...
function ToolbarAction({ title, label, icon, onClick, showLabel, active = false, danger = false }: ToolbarTool & { showLabel: boolean }) {
    ...
    return (
        <Tooltip title={title} placement="top" mouseEnterDelay={0.2}>
            <button type="button" className={`group relative flex h-12 items-center whitespace-nowrap px-1.5 ${danger ? "text-danger" : ""}`} onClick={onClick} aria-label={title}>
                <span
                    className={`flex h-9 items-center ${hasText ? "gap-2 px-2.5" : "justify-center px-2"} rounded-lg transition group-hover:bg-(--hover-bg) ${active ? "bg-(--hover-active)" : ""}`}
                    style={{ "--hover-bg": theme.toolbar.itemHover, "--hover-active": theme.toolbar.activeBg } as CSSProperties}
                >
                    {icon}
