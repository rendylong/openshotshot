...
import { isRemoteResultRetry, remoteTaskErrorText, remoteTaskForNode, selectNodeRemoteTaskDisplay } from "@/lib/canvas/remote-media-task-ui";
import { useProjectAssetUrl } from "@/hooks/use-project-asset-url";
import { useThemeStore } from "@/stores/use-theme-store";
import { Spinner } from "@/components/ui/spinner";
import { CanvasResourceMentionTextarea } from "./canvas-resource-mention-textarea";
import { CanvasNodeType, type CanvasNodeData, type CanvasNodeImage, type CanvasNodeText, type CanvasRemoteTaskMetadata, type Position } from "@/types/canvas";
import type { CanvasNodeContext, CanvasPluginHost } from "@/types/canvas-plugin";
import type { CanvasResourceReference } from "@/lib/canvas/canvas-resource-references";
import { useTranslation } from "react-i18next";
import { FileNodeContent } from "./nodes/file-node";
...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    // Neutral selection tokens (stone per theme) keep the canvas free of the one
    // saturated blue that fought the amber/stone system.
    const selectionColor = theme.node.activeStroke;
        ...
        >
            ...
            <div
                ...
                style={{
                    ...
                    borderColor: isGroup ? (isGroupDropTarget || isActive ? selectionColor : theme.node.stroke) : hasImageContent ? imageBorderColor : isActive ? selectionColor : isRelated ? theme.node.muted : transparentBg ? "transparent" : theme.node.stroke,
                    borderStyle: isGroup ? "dashed" : "solid",
                    boxShadow: isGroupDropTarget ? `0 0 0 2px ${selectionColor}66, inset 0 0 0 999px ${selectionColor}10` : isActive ? `0 0 0 1px ${selectionColor}55` : isRelated ? `0 0 0 1px ${theme.node.muted}55, 0 18px 48px rgba(0,0,0,.14)` : undefined,
                }}
                onMouseDown={(event) => {
...
const nodeContentRenderers = {
    ...
    [CanvasNodeType.File]: (props) => props.pluginContext ? <FileNodeContent ctx={props.pluginContext} /> : <EmptyImageContent {...props} />,
...
function LoadingContent({ node, theme }: Pick<NodeContentRendererProps, "node" | "theme">) {
    const { t } = useTranslation();
    const remoteTask = remoteTaskForNode(node);
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3" style={{ color: theme.node.activeStroke }}>
            <Spinner className="size-8 text-inherit" />
            {remoteTask ? <RemoteTaskStatus task={remoteTask} /> : <span className="text-[10px] tracking-[0.2em]">{t("canvas.node.generating")}</span>}
        </div>
    );
}
...
function ErrorContent({ node, theme, onRetry }: Pick<NodeContentRendererProps, "node" | "theme" | "onRetry">) {
    ...
    const { t } = useTranslation();
    const display = selectNodeRemoteTaskDisplay(node);
    return (
        <div className="flex max-w-[260px] flex-col items-center gap-3 px-5 text-center">
            <div className="text-xs leading-5 text-red-300">{remoteTaskErrorText(display.task, display.errorDetails, t)}</div>
            <button
                type="button"
            ...
            >
                ...
                {t(isRemoteResultRetry(node, remoteTasks) ? "canvas.remoteTask.retryResult" : "canvas.node.retry")}
            </button>
        </div>
    );
}
        ...
        <BatchFrame batchCount={batchCount} batchExpanded={batchExpanded}>
            ...
            <div className="flex h-full w-full flex-col overflow-hidden rounded-3xl">
                {isEditingContent ? (
                    ...
                    <div className={`thin-scrollbar block h-full w-full overflow-y-auto bg-transparent font-mono ${paddingClass}`} style={textStyle} onWheel={(event) => event.stopPropagation()}>
                        <div className="flex min-h-full w-full flex-col justify-center whitespace-pre-wrap break-words">{content}</div>
                    </div>
                ) : primaryText ? (
        ...
        >
            {text.content ? (
                <>
                    ...
                    <button type="button" className="absolute right-2.5 top-2.5 flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium transition hover:bg-accent" style={{ color: theme.node.text }} onClick={(event) => (event.stopPropagation(), onSetPrimary())} onPointerDown={(event) => event.stopPropagation()}>
...
function TextSlotStatus({ text }: { text: CanvasNodeText }) {
    ...
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3 px-6 text-center" style={{ background: theme.node.fill, color: failed ? theme.node.text : theme.node.activeStroke }}>
            {failed ? <span className="text-xs leading-5">{text.errorDetails || t("canvas.node.failed")}</span> : loading ? <Spinner className="size-8 text-inherit" /> : <span className="text-xs">{t("apiErrors.noContent")}</span>}
...
function VideoNodeContent({ node, theme }: NodeContentRendererProps) {
    const { t } = useTranslation();
    // 项目文件资产：挂载/changed 事件驱动解析，重启后经桥读回（content 里的 blob: URL 会失效）；
    // 无 assetRef 时 hook 直接返回原 content，行为与旧实现一致。
    const project = useProjectAssetUrl(node.metadata?.assetRef, node.metadata?.content || "");
    ...
    // data-canvas-media：普通左键让给原生播放器控件；滚轮/中键/space 手势仍由画布接管平移缩放。
    return <video src={project.url} controls className="h-full w-full rounded-[18px] bg-black object-contain" data-canvas-video={node.id} data-canvas-media />;
}
...
function AudioNodeContent({ node, theme }: NodeContentRendererProps) {
    ...
    return (
        <div className="flex h-full w-full flex-col justify-center gap-3 px-4" style={{ background: theme.node.fill, color: theme.node.text }}>
            ...
            <audio src={project.url} controls className="w-full" data-canvas-media />
        </div>
    );
}
...
function ImageContent({
    ...
    // 项目文件资产（主图或节点级）：解析失败/未就绪时回退原 content，行为与旧实现一致。
    const project = useProjectAssetUrl(primaryImage?.assetRef ?? node.metadata?.assetRef, primaryContent || "");
    ...
    return (
        <BatchFrame batchCount={batchCount} batchExpanded={batchExpanded}>
            ...
            {primaryImage?.status === "error" ? <BatchImageFailureActions node={node} imageId={primaryImage.id} placement="left" onRetry={() => onRetryBatchImage?.(primaryImage.id)} onDelete={() => onDeleteBatchImage?.(primaryImage.id)} /> : null}
            {primaryImage?.content ? (
                <button type="button" className="absolute left-2.5 top-2.5 z-30 flex h-8 items-center gap-1 rounded-lg border px-2 text-[10px] font-medium shadow-[0_6px_18px_rgba(15,23,42,.16)] backdrop-blur-md transition hover:scale-[1.02]" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.toolbar.activeText }} title={t("common.download")} onClick={(event) => (event.stopPropagation(), onDownloadBatchImage?.(primaryImage.id))} onPointerDown={(event) => event.stopPropagation()}>
...
function ExpandedImageCard({ node, image, index, onView, onSetPrimary, onDuplicate, onDownload, onRetry, onDelete }: { node: CanvasNodeData; image: CanvasNodeImage; index: number; onView: () => void; onSetPrimary: () => void; onDuplicate: () => void; onDownload: () => void; onRetry: () => void; onDelete: () => void }) {
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    const { t } = useTranslation();
    // 项目文件资产：每张卡独立解析（挂载/changed 事件驱动，重启后经桥读回 content 里失效的 blob: URL）；
    // 无 assetRef 的旧图回退原 content，行为与旧实现一致。
    const project = useProjectAssetUrl(image.assetRef, image.content);
    ...
    return (
        ...
        >
            {image.content ? <img src={project.url} alt={node.title} draggable={false} className="pointer-events-none h-full w-full select-none object-contain" /> : <ImageSlotStatus image={image} />}
            {image.content ? (
                <div className="absolute inset-x-2 top-2 flex items-center gap-1">
                    <button type="button" className="flex h-8 min-w-0 flex-1 items-center justify-center gap-1 rounded-lg border px-1.5 text-[10px] font-medium shadow-[0_6px_18px_rgba(15,23,42,.16)] backdrop-blur-md transition hover:scale-[1.02]" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.toolbar.activeText }} title={t("common.download")} onClick={(event) => (event.stopPropagation(), onDownload())} onPointerDown={(event) => event.stopPropagation()}>
                        <Download className="size-3 shrink-0" />
                        <span className="truncate">{t("common.download")}</span>
                    </button>
                    <button type="button" className="flex h-8 min-w-0 flex-1 items-center justify-center gap-1 rounded-lg border px-1.5 text-[10px] font-medium shadow-[0_6px_18px_rgba(15,23,42,.16)] backdrop-blur-md transition hover:scale-[1.02]" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.toolbar.activeText }} title={t("canvas.node.createCopy")} onClick={(event) => (event.stopPropagation(), onDuplicate())} onPointerDown={(event) => event.stopPropagation()}>
            ...
            {image.status === "error" ? <BatchImageFailureActions node={node} imageId={image.id} placement="right" onRetry={onRetry} onDelete={onDelete} /> : null}
        </div>
    );
}

function BatchImageFailureActions({ node, imageId, placement, onRetry, onDelete }: { node: CanvasNodeData; imageId: string; placement: "left" | "right"; onRetry: () => void; onDelete: () => void }) {
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    const remoteTasks = useRemoteMediaTaskStore((state) => state.tasks);
    const { t } = useTranslation();
    return (
        <div className={`absolute top-3 z-30 flex items-center gap-1.5 ${placement === "left" ? "left-3" : "right-3"}`}>
            <button type="button" className="flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-xs font-medium shadow-sm transition hover:scale-[1.02]" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.text }} onClick={(event) => (event.stopPropagation(), onRetry())} onPointerDown={(event) => event.stopPropagation()}>
                <RefreshCw className="size-3.5" />
                {t(isRemoteResultRetry(node, remoteTasks, imageId) ? "canvas.remoteTask.retryResult" : "canvas.node.retry")}
            </button>
...
function ImageSlotStatus({ image }: { image?: CanvasNodeImage }) {
    ...
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3 px-6 text-center" style={{ background: theme.node.fill, color: failed ? theme.node.text : theme.node.activeStroke }}>
            {failed ? <span className="text-xs leading-5">{remoteTaskErrorText(image?.remoteTask, image?.errorDetails, t)}</span> : <Spinner className="size-8 text-inherit" />}
            {!failed ? image?.remoteTask ? <RemoteTaskStatus task={image.remoteTask} /> : <span className="text-[10px] tracking-[0.2em]">{t("canvas.node.generating")}</span> : null}
        </div>
    );
}

function RemoteTaskStatus({ task }: { task: CanvasRemoteTaskMetadata }) {
    const { t } = useTranslation();
    ...
    const status = task.status === "submitting"
        ? t("canvas.remoteTask.submitting")
        : task.status === "waiting_network"
          ? t("canvas.remoteTask.waitingNetwork")
          : task.status === "waiting_configuration"
            ? t("canvas.remoteTask.waitingConfiguration")
            : task.recoveryPhase
              ? t(task.recoveryPhase === "confirming" ? "canvas.remoteTask.confirmingSubmission" : "canvas.remoteTask.fetchingResult")
              : t(`canvas.remoteTask.${task.phase ?? "queued"}`);
    const elapsed = Math.max(0, Math.floor((now - task.submittedAt) / 1_000));
    return (
        <div className="flex flex-col items-center gap-1 text-center text-[10px] leading-4">
            <span className="font-medium">{status}{typeof task.progress === "number" ? ` · ${t("canvas.remoteTask.progress", { progress: Math.round(task.progress) })}` : ""}</span>
            <span className="opacity-60">{t("canvas.remoteTask.elapsed", { seconds: elapsed })}</span>
        </div>
    );
}
...
function ResizeHandle({ corner, onMouseDown }: { corner: ResizeCorner; onMouseDown: (event: React.MouseEvent, corner: ResizeCorner) => void }) {
    ...
    return <div className={`absolute z-50 size-7 ${positionClass}`} onMouseDown={(event) => onMouseDown(event, corner)} onPointerDown={(event) => event.stopPropagation()} />;
}
