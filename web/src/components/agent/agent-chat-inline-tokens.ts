...
export function parseAgentInlineTokens(text: string, references: AgentCanvasReference[], skillNames: string[] = []): AgentInlineToken[] {
    const markers = [
        ...references.map((reference) => ({ marker: agentReferenceMarker(reference), token: { type: "reference", reference } as AgentInlineToken })),
        ...skillNames.map((name) => ({ marker: `/${name}`, token: { type: "skill", name } as AgentInlineToken })),
