...
import { type AgentAttachment, type AgentChatItem, type AgentEventLog, type AgentMessageAttachment, type AgentTokenUsage } from "@/stores/use-agent-store";
...
export type AgentUserDetail = { toolName?: string; kind: string; status: string; rows?: Array<{ label: string; value: string }>; input?: string; output?: string; files?: Array<{ path: string; action?: string }>; tasks?: Array<{ step: string; status: string }>; explanation?: string; startedAt?: number; completedAt?: number; durationMs?: number };
...
export function agentMessageToChatMessage(item: AgentChatItem) {
    return { ...item, meta: item.role === "user" || item.role === "assistant" ? undefined : item.meta, attachments: item.attachments?.map((attachment) => agentAttachmentToChatAttachment(attachment)) };
}

export function agentAttachmentToChatAttachment(item: AgentMessageAttachment): AgentChatAttachment {
    // kind/assetRef/relativePath 是持久身份：恢复历史时也要原样带出，禁止退化成纯图片。
    return { id: item.id, name: item.name, url: item.dataUrl || item.url, kind: item.kind, mimeType: item.mimeType, size: item.size, dataUrl: item.dataUrl, handle: item.handle, assetRef: item.assetRef, relativePath: item.relativePath };
}

export function formatAgentEvent(event: AgentEventPayload): Omit<AgentChatItem, "id"> | null {
    const item = event.item;
    if (event.type === "item.completed" && item?.type === "agent_message") return { role: "assistant", title: "shotshot", text: stringText(item.text) };
    return null;
}
...
export function toolName(name: string, fallback?: string) {
    if (name === "wait") return tr("tools.wait");
    if (name === "read") return tr("tools.readFile");
    if (name === "bash") return tr("tools.executeCommand");
    if (name === "edit") return tr("tools.editFiles");
    if (name === "write") return tr("tools.writeFile");
    if (name === "generate_chatgpt_image") return tr("tools.generateChatGptImage");
...
export function toolDetail(item: AgentEventItem | undefined, status: string): AgentUserDetail {
    const name = String(item?.tool || "");
    return { kind: "tool", toolName: name, status, rows: toolInputRows(name, item?.arguments), ...(item?.error?.message ? { output: item.error.message } : {}) };
}

export function toolCallDetail(name: string, input: unknown, status: string, error = ""): AgentUserDetail {
    return { kind: "tool", toolName: name, status, rows: toolInputRows(name, input), ...(error ? { output: error } : {}) };
}
...
function toolInputRows(name: string, input: unknown) {
    ...
    if (name === "bash") return [detailRow(tr("command"), stringText(objectField(input, "command")))].flatMap((row) => (row ? [row] : []));
    if (name === "edit" || name === "write") return [detailRow(tr("filePath"), stringText(objectField(input, "path")))].flatMap((row) => (row ? [row] : []));
    if (name === "canvas_create_text_node") return [detailRow(tr("textContent"), objectField(input, "text"))].flatMap((row) => (row ? [row] : []));
    if (name === "canvas_create_text_nodes") return [detailRow(tr("textContent"), Array.isArray(objectField(input, "items")) ? `${(objectField(input, "items") as unknown[]).length} 条` : "")].flatMap((row) => (row ? [row] : []));
    ...
    if (name === "skill_read") {
        ...
        const skillName = stringText(objectField(input, "name"));
        if (skillName) args.push({ label: tr("inputSkillName") || "Skill", value: skillName });
        const path = stringText(objectField(input, "path"));
        if (path) args.push({ label: tr("inputPath") || "Path", value: path });
        return args.map((row) => detailRow(row.label, row.value)).flatMap((row) => (row ? [row] : []));
    }
    if (name === "canvas_get_state" || name === "canvas_export_snapshot") return [];
    if (input == null) return [];
    const serialized = typeof input === "string" ? input : JSON.stringify(input);
    if (!serialized || serialized === "{}" || serialized === "[]") return [];
    return [detailRow(tr("inputArgs"), serialized)].flatMap((row) => (row ? [row] : []));
}
...
export function toolSummary(item?: AgentEventItem) {
    ...
    if (name === "skill_read") {
        const text = stringText(objectField(result, "text"));
        if (!text) return "";
...
export function routeName(path: string) {
    ...
    if (path.startsWith("/skills")) return tr("routes.skills");
    if (path.startsWith("/projects")) return tr("routes.projects");
    return path;
}
...
export function workingActivity(item?: AgentChatItem) {
    ...
    const rawTool = String(objectField(item?.detail, "toolName") || "");
    const label = rawTool ? toolName(rawTool) : (item.title || tr("toolOperation"));
    if (["inProgress", "in_progress", "running", "pending"].includes(status)) return { key, text: tr("operationRunning", { operation: label }) };
    if (rawTool === "canvas_get_state") return { key, text: tr("organizingCanvas") };
    return { key, text: tr("operationCompleted", { operation: label }) };
}

export function isRunningWaitItem(item?: AgentChatItem) {
    if (item?.role !== "tool") return false;
    if (String(objectField(item?.detail, "toolName") || "") !== "wait") return false;
    return ["inProgress", "in_progress", "running", "pending"].includes(String(objectField(item?.detail, "status") || ""));
}
