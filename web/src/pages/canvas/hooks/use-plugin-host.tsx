...
type PluginHostParams = {
    ...
    open3dPreview: (nodeId: string) => void;
    /** 项目资产写上下文（画布页显式携带）：插件宿主生成产物经门面落项目工作区；无项目上下文时缺省。 */
    assetWriteContext?: (source: ProjectAssetSource) => ProjectAssetWriteContext;
};
...
export function usePluginHost(params: PluginHostParams) {
    ...
    const { effectiveConfig, isAiConfigReady, openConfigDialog, theme, nodesRef, connectionsRef, viewportRef, setNodes, setDialogNodeId, applyAgentOps, open3dPreview, assetWriteContext } = params;
    ...
    const pluginAi = useMemo<CanvasPluginAi>(() => {
        ...
        return {
            ...
            generateVideo: async (prompt, options) => {
                ...
                ensureReady(config);
                // 生成产物经项目资产门面落项目工作区（无项目上下文时回退 IndexedDB，行为同前）。
                const file = await storeGeneratedVideo(await requestVideoGeneration(config, prompt, toReferences(options?.references), { signal: options?.signal }), assetWriteContext?.({ type: "generated" }));
    ...
    }, [assetWriteContext, effectiveConfig, isAiConfigReady, openConfigDialog, t]);
    ...
    const pluginHost = useMemo<CanvasPluginHost>(
        ...
        [applyAgentOps, pluginAi, open3dPreview, assetWriteContext],
    );
