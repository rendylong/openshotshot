...
export type CanvasNodeContext = {
    ...
    // Opens a fullscreen viewer to browse this 3D node's content; the host renders the viewer and a "zoom back" affordance.
    open3dPreview: () => void;
    ...
    // 画布页注入的项目资产写上下文工厂；缺失（Web/宿主未注入）时插件自行回退 IndexedDB 直写。
    assetWriteContext?: (source: ProjectAssetSource, nodeId?: string) => CanvasAssetWriteInput;
};
...
export type CanvasPluginHost = {
    ...
    applyOps: (ops: CanvasAgentOp[]) => void;
    // Opens a fullscreen viewer to browse a 3D node's content (see CanvasNodeContext.open3dPreview).
...
export type CanvasNodeDefinition = {
    ...
    // Static reference capability for canvas/agent affordances (mention eligibility, agent referenceKind hints).
    // Declared instead of derived from resource() so it stays reliable before the node mounts (cold cache).
    referenceKind?: "image" | "video" | "audio" | "text";
